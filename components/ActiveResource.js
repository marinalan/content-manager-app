...
const ActiveResource = () => {
  ...
  useEffect(() => {
    async function fetchResource() {
      ...
      const timeToFinish = parseInt(resData.timeToFinish, 10);
      const elapsedTime = moment().diff(moment(resData.activationTime), "seconds");
      
      const updatedTimeToFinish = (timeToFinish*10) - elapsedTime;

      if (updatedTimeToFinish >= 0) {
        resData.timeToFinish = updatedTimeToFinish;
